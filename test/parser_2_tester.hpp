...
struct Parser2Tester final : public protocol::json::Parser2::Handler {
 ...
 protected:
  ...
  void operator()(Trace<protocol::json::Orderbook> const &event) override { dispatch(event); }
  void operator()(Trace<protocol::json::Trades2> const &event) override { dispatch(event); }
  void operator()(Trace<protocol::json::Market24h2> const &event) override { dispatch(event); }
  void operator()(Trace<protocol::json::Kline2> const &event) override { dispatch(event); }
  void operator()(Trace<protocol::json::IndexMarket24h> const &event) override { dispatch(event); }
  void operator()(Trace<protocol::json::AccountsOrdersPositions2> const &event) override { dispatch(event); }
