...
namespace {
auto map_time_in_force(auto time_in_force, auto execution_instructions) -> protocol::json::TimeInForce {
...
std::string_view Encoder::orders_create_coin_m(
  ...
  auto pos_side = map(create_order.position_effect, create_order.side).template get<protocol::json::PosSide>();
  auto ord_type = map(create_order.order_type).template get<protocol::json::OrderType>();
...
std::string_view Encoder::orders_replace_usd_m(
  ...
  auto pos_side = map(order.position_effect, order.side).template get<protocol::json::PosSide>();
  fmt::format_to(
      std::back_inserter(buffer),
      "&symbol={}"
      "&posSide={}"sv,
